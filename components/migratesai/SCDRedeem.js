...
export default ({ onNext, onPrev }) => {
  ...
  const [amount, setAmount, onAmountChange, amountErrors] = useValidatedInput(
    ...
    {
      maxFloat: () => 'Insufficient SAI balance'
    }
  );
  ...
  return (
    <Box maxWidth="71.8rem" mx={['s', 0]}>
      ...
      <Text textAlign="center" mb="xl">
        How much Single-collateral Dai would you like to upgrade to
      ...
      <Card px={{ s: 'l', m: '2xl' }} py="l" mb="xl">
        ...
        <Input
          ...
          after={
            <Link
              fontWeight="medium"
              onClick={() => setAmount(saiBalance.toNumber())}
            >
              Set max
            </Link>
          }
        />
        <Text.p>Sai Balance</Text.p>
        <Text.p>
          {saiBalance ? `${saiBalance.toNumber().toFixed(2)} SAI` : '...'}
        </Text.p>
      </Card>
      <Card px={{ s: 'l', m: '2xl' }} py="l" mb="xl">
        <Text.p>SAI to DAI exchange rate</Text.p>
        <Text.p>1:1</Text.p>
        <Text.p>Max SAI to DAI Liquidity</Text.p>
        <Text.p>

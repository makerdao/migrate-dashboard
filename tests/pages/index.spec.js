...
const mockHardwareFactory = jest.fn(maker => {
  maker.service('accounts', true).addAccountType('ledger', async settings => {
    ...
    if (settings.accountsLength && settings.accountsLength > 1) {
      ...
      const addresses = await new Promise(resolve =>
        setTimeout(()=>resolve(mockAddresses), 100)
      );
...
test('select trezor address', async () => {
  ...
  setTimeout(()=>getByText(/Select address/),200);
});

test.only('select ledger address', async done => {
  let maker;
  ...
  //change back to wait, just for simplicity?
  setTimeout(()=>{
    getByText(/Select address/);
    done();
  },200);
});